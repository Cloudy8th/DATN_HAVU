...
function normalizeOp(op = "eq") {
  ...
  if (o === "startswith" || o === "starts_with" || o === "^")
    return "startswith";
...
export function planToSQL(plan) {
  ...
  // Helper: build "alias"."column" từ field (có prefix hoặc không)
  const qcol = (qualified) => {
    const { res, field } = splitField(qualified);
    const alias = res ? aliasMap[res] : baseAlias;
    if (!alias) {
      // nếu thiếu alias do không JOIN, cứ trỏ về bảng gốc để tránh crash
  ...
  for (const w of plan.where || []) {
    ...
    // meta để chọn cast đúng (uuid[]/numeric[]/text[])
    let meta;
    if (!res) {
    ...
    switch (op) {
      ...
      case "in": {
        ...
        const cast =
          t === "uuid" ? "uuid[]" : t === "number" ? "numeric[]" : "text[]";
  ...
  if (plan.sort?.length) {
    const parts = plan.sort
      .map((s) => {
        return `${qcol(s.field)} ${String(s.dir || "asc").toUpperCase()}`;
      })
      .join(", ");
    if (parts) orderSql = ` ORDER BY ${parts}`;
  } else {
    // ví dụ auto sort theo discount desc nếu có lọc discount > / >=
    const hasDiscountFilter = (plan.where || []).some(
      (w) =>
        splitField(w.field).field === "discount" &&
        ["gt", "gte"].includes(normalizeOp(w.op))
    );
    if (hasDiscountFilter && baseRes.fields["discount"]) {
      orderSql = ` ORDER BY ${qcol("discount")} DESC`;
    }
  }
