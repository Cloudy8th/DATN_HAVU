...
// helper: "Category.name" -> { res: "Category", field: "name" }
...
export function validatePlan(plan) {
  ...
  // 2) hợp thức hoá JOIN theo whitelist trong CONTRACT.resources[baseKey].relations
  const requestedJoins = Array.isArray(plan.joins) ? plan.joins : [];
  const joins = [];
  if (requestedJoins.length) {
    // giới hạn số join để an toàn (tuỳ bạn)
    const maxJoins = 2;
    for (const j of requestedJoins.slice(0, maxJoins)) {
      const jr = j?.resource;
      if (!jr) continue;
      const rel = base.relations?.[jr];
      if (!rel) continue; // không được whitelist -> bỏ
      const frKey = rel.foreignResource || jr;
      if (!CONTRACT.resources[frKey]) continue; // resource đích phải tồn tại
      joins.push({
        resource: jr, // tên resource join "Category"
        alias: rel.alias || jr[0].toLowerCase(), // alias mặc định
        type: (rel.type || "inner").toLowerCase(), // "inner" | "left"
        localField: rel.localField, // vd "category_id"
        foreignResource: frKey, // "Category"
        foreignField: rel.foreignField, // vd "id"
      });
    }
  }

  // helper: kiểm tra field hợp lệ ở base hoặc ở resource join
  ...
  // 3) SELECT (fallback defaultSelect của base; chấp nhận "Res.field")
  let select =
  ...
  // 4) WHERE (chỉ nhận field hợp lệ; op/value xử lý ở build-SQL)
